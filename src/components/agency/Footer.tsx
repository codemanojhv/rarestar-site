  ...
  return (
    <footer className="relative border-t border-paper/10 bg-ink px-6 py-16 md:px-10 md:py-20">
      <div className="mx-auto max-w-7xl">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-2.5">
              <Image
                ...
                className="h-6 w-6"
              />
              <span className="font-mono text-[12px] uppercase tracking-[0.22em] text-paper/80">
                RareStar
              </span>
            </div>
            <p className="max-w-xs text-sm text-paper/50 font-sans">
        ...
        {/* Grid — Nav + Social + Booking */}
        <div className="mt-12 grid gap-8 border-t border-paper/10 pt-12 md:grid-cols-3">
          <nav aria-label="Footer navigation">
            <h3 className="mb-4 font-mono text-[10px] uppercase tracking-[0.22em] text-paper/40">Navigation</h3>
            <ul className="flex flex-col gap-2">
              {NAV_LINKS.map((l) => (
                <li key={l.id}>
                  <a href={`#${l.id}`} onClick={handleNav(l.id)} className="font-sans text-sm text-paper/60 transition-colors hover:text-paper">
                    {l.label}
                  </a>
                </li>
              ))}
          ...
          <div>
            <h3 className="mb-4 font-mono text-[10px] uppercase tracking-[0.22em] text-paper/40">Connect</h3>
            <ul className="flex flex-col gap-2">
              {SOCIAL_LINKS.map((l) => (
                <li key={l.label}>
                  <a href={l.href} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 font-sans text-sm text-paper/60 transition-colors hover:text-paper">
                    <span>{l.label}</span>
          ...
          <div>
            <h3 className="mb-4 font-mono text-[10px] uppercase tracking-[0.22em] text-paper/40">Availability</h3>
        ...
        <div className="mt-12 flex flex-col gap-4 border-t border-paper/10 pt-8 md:flex-row md:items-center md:justify-between">
          <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-paper/30">
            © {year} RareStar. All rights reserved.
          </p>
          <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-paper/30">
