...
function HeroContent({
  ...
  return (
    <div
      className={[
        "absolute inset-0 flex h-full flex-col items-start justify-end px-6 pb-20 text-left md:px-12 md:pb-24",
      ...
      <div className="flex w-full max-w-7xl flex-col items-start">
        ...
        >
          <Image
            ...
            height={110}
            priority={!ink}
            className="h-auto w-[min(72vw,360px)] md:w-[440px]"
          />
        </div>
        ...
        <TitleTag
          data-reveal-target={ink ? undefined : ""}
          data-hover={ink ? undefined : "reveal"}
          className={`display ${titleColor} text-left text-[12vw] leading-[0.9] md:text-[8.5vw] md:leading-[0.85]`}
        >
        ...
        <div data-hero-tagline className="mt-10 flex w-full flex-col items-start">
          <p
            data-reveal-target={ink ? undefined : ""}
            data-hover={ink ? undefined : "reveal"}
            className={`${revealUp} max-w-md text-left font-sans text-base leading-relaxed md:text-lg ${taglineColor}`}
          >
            {TAGLINE}
          </p>
          ...
          <div className={`mt-10 flex w-full items-center justify-end gap-6 ${revealUp} ${ink ? "opacity-0" : ""}`}>
            ...
            <a
              href="#contact"
              onClick={handleCTA}
              className="group inline-flex items-center gap-3 rounded-full bg-ember px-8 py-4 font-mono text-[11px] uppercase tracking-[0.25em] text-paper transition-all hover:bg-paper hover:text-ink md:px-10"
            >
...
export default function Hero() {
  ...
  return (
    <section
      ...
      aria-label="Rarestar Agency"
    >
      <HeroVideo />
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-ink/65 via-ink/10 to-ink/92" />

      <div
        ref={revealRef}
        className="reveal-root absolute inset-0 z-10"
        style={{ "--cx": "50%", "--cy": "50%", "--cr": "0px" } as React.CSSProperties}
      >
        <HeroContent animate />
        <div aria-hidden="true" className="reveal-blob pointer-events-none absolute rounded-full bg-ember shadow-[0_0_60px_rgba(207,0,0,0.28)]" />
      ...
      <div
        data-hero-scroll
        className="reveal-up absolute bottom-8 left-1/2 z-10 flex -translate-x-1/2 flex-col items-center gap-3 font-mono text-[10px] uppercase tracking-[0.4em] text-paper/40"
      >
        <span>Scroll</span>
        <span className="inline-block h-12 w-px animate-pulse bg-paper/20" />
      </div>
    </section>
  );
}
